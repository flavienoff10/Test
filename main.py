...
# === IMPORTS DIRECTS (plus d'install auto) ===
    ...
    def report(self):
        while True:
            ...
            if "Thanks for your feedback" in response.text or response.status_code == 200:
                ...
                self._clear()
                print(f"{StaticValues.SUCCESS}{self.victim_data['nickname']} Reported {StaticValues.REPORT_COUNT} Times! ({(StaticValues.REPORT_COUNT/StaticValues.TOTAL_REQUESTS)*100:.1f}% Success Rate)")
            else:
                print(f"{StaticValues.WARNING}Error ({(StaticValues.REPORT_COUNT/StaticValues.TOTAL_REQUESTS)*100:.1f}% Success Rate)")
